...
-   Architectural governance — ensuring architectural decisions are respected over time via automated fitness functions rather than manual reviews; ArchUnit, NetArchTest, chaos engineering; checklist manifesto framing
...
-   [Architecture quantum]({{< relref "architecture_quantum.md" >}}) — smallest independently deployable unit scoping a set of architectural characteristics; four properties (independent deployment, high functional cohesion, low static coupling, synchronous communication); DDD bounded context as domain analogue
-   [Component-based thinking]({{< relref "component_based_thinking.md" >}}) — seeing systems as logical components (above class level); Workflow and Actor/Action identification approaches; Entity Trap antipattern; Law of Demeter for coupling reduction
-   [Architecture Styles — Foundations]({{< relref "architecture_styles_foundations.md" >}}) — styles vs patterns distinction; Big Ball of Mud antipattern; technical vs domain partitioning; Conway's Law + Inverse Conway Maneuver; all 11 fallacies of distributed computing; team topologies (Skelton & Pais)
