:PROPERTIES:
:ID:       2e0c582e-2cf0-4e2c-a1bd-378b6041a3ce
:END:
#+title: LLM wiki
...
[[id:8c713807-89e7-42cb-bdf4-cb3f53d620f2][second brain]] / [[id:ea3192d3-7186-4e4a-8c98-454d318d532b][Zettelkasten]] methods for the 50–200 source-document scale,
...
- [[id:8c713807-89e7-42cb-bdf4-cb3f53d620f2][Second brain]] — LLM wiki as a next-generation second-brain implementation
- [[id:ea3192d3-7186-4e4a-8c98-454d318d532b][Zettelkasten]] — atomic-note linking philosophy; LLM wiki automates the linking
- [[id:5329cb10-9f3e-4311-bc69-542775991244][org-roam]] — Emacs-based Zettelkasten; org-roam wikis share the index+backlink navigation pattern
- [[id:8f1e78cd-fefa-4a2f-9db9-0e1e396a0448][AI]] — language models as the engine behind the pattern
- [[id:f2b593c9-52ec-41ca-b376-b95d227d782c][Claude Code skills]] — the LLM wiki pattern has been implemented as a Claude Code skill (github.com/6eanut/llm-wiki)
...
- *qmd* ([[https://github.com/tobi/qmd][github.com/tobi/qmd]]) — local search engine for markdown files with hybrid BM25/vector search and LLM re-ranking; has both CLI and MCP server; the right tool once the index file approach starts to strain at larger scale.
...
- 2026-06-12 ◦ [[https://github.com/6eanut/llm-wiki][llm-wiki (GitHub)]] — open-source Claude Code skill implementing the Karpathy pattern; workflow: drop notes into ~.raw/~ → agent ingests, extracts concepts, writes linked markdown pages; proactive querying (agent checks wiki before falling back to training data); includes slash commands (~ingest~, ~query~, ~lint~) and a D3 knowledge graph; the project itself is markdown files and bash scripts, so the agent can read its own schema and follow its own procedures

* Resources
- 2026-06-02 ◦ [[https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f][LLM wiki gist (Karpathy)]] — original specification by Andrej Karpathy; intentionally abstract and designed to be shared with an LLM agent to instantiate a concrete version
- 2026-06-02 ◦ [[https://blog.starmorph.com/blog/karpathy-llm-wiki-knowledge-base-guide][How to build Karpathy's LLM wiki]] — comprehensive guide covering three-layer architecture, ingest/query/lint operations, LLM wiki vs RAG comparison, and community implementations
- 2026-06-03 ◦ CLAUDE.md as team infrastructure — [[id:1d69111a-a3c1-4c18-af2a-b6a2ad6c04c8][Claude Code]] extends the CLAUDE.md-as-schema-file concept into shared team conventions, domain glossary, and guardrails; the same schema-as-code philosophy applied to a multi-developer context

- 2026-06-18 ◦ [[https://www.reddit.com/r/hermesagent/comments/1tlh53c/am_i_missing_the_point_of_ai_agents/][Am I missing the point of AI agents? (Reddit r/hermesagent)]] — a practitioner describes a personal knowledge base [[id:c4cdacbd-1bde-4ddd-866b-c8762e56e0b4][AI agent]] that "finds connections, opportunities and news and finds paths that I likely would not have found manually" — serendipitous cross-topic discovery as the primary value over simple retrieval
