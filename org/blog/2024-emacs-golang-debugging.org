...
In order to debug our Task API we have 2 approaches
- we can debug the application directly
- we can attach to a running process

Now let's delve into each workflow.
...
Let's say you now want to attach the debugger to a running process (remotely or locally):

#+captionn: Additional config for attaching to a running process
#+begin_src emacs-lisp :tangle (org-sbe code-dir (filename \".dir-locals.el\"))
((go-mode . ((dape-configs .
   ((go-debug-taskapi
     modes (go-mode go-ts-mode)
     ...
     :showLog "true"
     :program ".")
    ;; New profile for attaching to running processes
    (go-attach-taskapi
     modes (go-mode go-ts-mode)
     command "dlv"
     command-cwd default-directory
...
Now let's start the *debugger*:
#+begin_src shell
~/repos/priv/blog/static/code/2024/emacs-golang-debugging $ go build -o taskapi *.go
~/repos/priv/blog/static/code/2024/emacs-golang-debugging $ dlv debug taskapi --listen=localhost:55878 --headless
...
** TODO Launch debug server independently
