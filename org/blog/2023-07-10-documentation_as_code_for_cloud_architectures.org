...
In order to have a concrete example, let's implement a *self-destructing Email service* that
allows users to send *self-destructing* emails a la "Mission Impossible"  😎. The /software architecture/ for the self-destructing email service consists of following components:
...
Let's add more complexity and use {{{zk_topic(microservices,microservices)}}} along with [[https://aws.amazon.com/eks/][AWS EKS]]. The infrastructure for the self-destructing email service, using AWS and EKS could consist of following components:
