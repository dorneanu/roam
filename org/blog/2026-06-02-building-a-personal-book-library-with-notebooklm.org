:PROPERTIES:
:ID:       F3A1B2C4-D5E6-7890-ABCD-EF1234567891
:END:
#+title: Building a personal book library with NotebookLM and Claude Code
#+filetags: :ai:books:notebooklm:claudecode:
#+author: Victor Dorneanu
#+date: 2026-06-02
#+setupfile: setup.org
#+draft: false

Ever since I started highlighting text on my Kindle, then my PocketBook, and also the [[/2021/06/13/note-taking-in-2021/][old-fashioned way with pen and paper]], I have been trying to store those highlights in my [[https://brainfck.org][digital garden]] — which serves as a *second brain* for recalling insights from books I have read. I built [[https://brainfck.org][brainfck.org]] to keep my collection of notes available online, not only for myself but also for others. I even added a [[https://brainfck.org/books/][search feature]] (first introduced in [[https://github.com/dorneanu/roam/commit/ce365029a143dad9b8bdb11f8484c0a8ba97b017][ce36502]] and later improved in [[https://github.com/dorneanu/roam/commit/9a23b40][9a23b40]]) that lets you search across the entire collection. Whenever I want to look something up, I go to [[https://brainfck.org][brainfck.org]], type in a keyword, and it shows me the books and journal entries where that word appears.

Recently I built a workflow that allows to expand all this. I now have all my /e-books/ loaded into [[https://notebooklm.google.com][NotebookLM]] notebooks, organised year by year, so I can ask questions across everything I have read in a given period. The process is to convert EPUBs and
...
#+html: {{< gbox src="/posts/img/2026/building-personal-book-library-notebooklm/notebooklm-notebooks-overview.png" title="NotebookLM notebooks overview" caption="One notebook per year — Books 2020 through Books 2026 — each populated with the full text of every book read that year." pos="left" >}}

* The tools

I used three MCP servers inside Claude Code:

- [[https://github.com/jacob-bd/notebooklm-mcp-cli][notebooklm-mcp]] — create notebooks, add sources, query them
- [[https://mcp.dropbox.com][DropboxMCP]] — search and download EPUBs/PDFs from my PocketBook library synced to Dropbox
- *WebFetch* — scrape [[https://brainfck.org/books/][brainfck.org/books]] to get the canonical list of books per year
...
The [[https://brainfck.org/books][brainfck books page]] lists the books I have read, sorted by year. Starting with only two notebooks ("Books 2026" and "Books 2025"), I wanted to create notebooks for earlier years but I did not know which books I actually had as EPUBs in Dropbox.

#+html: {{< gbox src="/posts/img/2026/building-personal-book-library-notebooklm/brainfck-books-page.png" title="brainfck.org/books" caption="The books page on brainfck.org — books grouped by year with read dates and tags. This served as the ground truth for which books belong in which notebook." pos="left" >}}

#+caption: Use claude-code to WebFetch the list of books
#+begin_src mcp
